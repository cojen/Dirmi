...
import dirmi.bench.ConfigFormatException;
import dirmi.bench.Harness;
import dirmi.bench.HtmlReporter;
import dirmi.bench.Reporter;
import dirmi.bench.TextReporter;
import dirmi.bench.XmlReporter;
...
public class Main {
    ...
    static final String CONFFILE = "/dirmi/bench/rmi/config";
    ...
    static void setupServer() {
        ...
        switch (runmode) {
            case SAMEVM:
                ...
                break;
                */
                die("Unsupported. Use client/server mode");

            case CLIENT:
                try {
                    server = (BenchServer) env.newSessionConnector(host, port).connect().receive();
                } catch (Exception e) {
            ...
            case SERVER:
                try {
                    env.newSessionAcceptor(port).acceptAll(new BenchServerImpl());
                } catch (Exception e) {
